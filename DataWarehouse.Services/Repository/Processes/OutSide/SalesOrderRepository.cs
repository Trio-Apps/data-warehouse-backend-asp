   ...
   int pageSize = 20)
        ...
        (int warehouseId, int pageNumber, int pageSize, CancellationToken cancellationToken = default)
    {
        ...
        var data = await query
             .OrderByDescending(so => so.SalesOrderId) // ãåã áËÈÇÊ ÇáÜ pagination
             .Skip((pageNumber - 1) * pageSize)
             .Take(pageSize)
             .Select(iw => new
             {
    ...
    {
        // await reasonValidationService.ValidateAsync(dto.ReasonId, ProcessType.Sales);
        ...
        if (Enum.TryParse<GeneralStatus>(status, out var statusEnum))
        {
            var query = await Query().Where(so => so.Status == statusEnum)
                .Select(so => new SalesOrderDTO
                {
                    ...
                    Comment = so.Comment,
                    ReasonId = so.ReasonId,
                    ReasonName = so.Reason != null ? so.Reason.Name : null
                }).ToListAsync();
