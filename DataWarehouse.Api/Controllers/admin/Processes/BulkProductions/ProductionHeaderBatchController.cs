...
[Route("api/production-header-batches")]
[ApiController]
[Authorize]
public class ProductionHeaderBatchController : ControllerBase
{
    private readonly IProductionHeaderBatchRepository _repository;

    public ProductionHeaderBatchController(IProductionHeaderBatchRepository repository)
    {
    ...
    public async Task<IActionResult> Create([FromBody] AddProductionHeaderBatchDTO dto)
    {
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateProductionHeaderBatchDTO dto)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);
