...
[Route("api/production-component-lines")]
[ApiController]
[Authorize]
public class ProductionComponentLineController : ControllerBase
{
    private readonly IProductionComponentLineRepository _repository;

    public ProductionComponentLineController(IProductionComponentLineRepository repository)
    {
    ...
    public async Task<IActionResult> Create([FromBody] AddProductionComponentLineDTO dto)
    {
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateProductionComponentLineDTO dto)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);
